...
public class CourseService {
    ...
    public Optional<Course> findByIdAndAddCourse(int courseId, int activityId)
    {
        Optional<Course> course= courseRepository.findById(courseId);
        if(course.isPresent())
        {
            ArrayList<Integer> activityIdList=course.get().getActivityIdList();
            activityIdList.add(activityId);
            this.saveCourse(course.get());
            return course;
        }
        else
            return null;
